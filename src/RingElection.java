...
public class RingElection {
    ...
    String[] address; /*
                       * Initialize the array to the IP addresses of the next occuring nodes in order
                       */
    ...
    int[] port; /*
                 * Initialize the array to the port number of the next occuring nodes in order
                 */
        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    for (int i = 0; i < n; i++) {
                        if (i != curPid) {
                            DatagramPacket sendPacket = new DatagramPacket(elByte, elByte.length,
                                    InetAddress.getByName(sAddress[i]), cPorts[i]);
            ...
            try {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        DatagramPacket sendPacket = new DatagramPacket(kMsg, kMsg.length, recvPacket.getAddress(),
                                recvPacket.getPort());
                ...
                while (true) {
                    ...
                    if (elNum == 0) {
                        ...
                        for (int i = 0; i < n - 1; i++) {
                            DatagramPacket recvPacket = new DatagramPacket(msg, msg.length);
                            DatagramPacket sendPacket = new DatagramPacket(elByte, elByte.length,
                                    InetAddress.getByName(address[i]), port[i]);
                    ...
                    while ((!queue.isEmpty())) {
                        ...
                        for (int i = 0; i < n - 1; i++) {
                            ...
                            DatagramPacket recvPacket = new DatagramPacket(msg, msg.length);
                            DatagramPacket sendPacket = new DatagramPacket(msByte, msByte.length,
                                    InetAddress.getByName(address[i]), port[i]);
                        ...
                        if (i != curPid - 1) {
                            try {
                                DatagramPacket sendPacket = new DatagramPacket(msg, msg.length,
                                        InetAddress.getByName(sAddress[i]), mPorts[i]);
